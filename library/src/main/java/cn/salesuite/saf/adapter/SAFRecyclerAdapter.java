...
public class SAFRecyclerAdapter<T,VH extends Presenter<T>> extends RecyclerView.Adapter<VH> {
    ...
    public static <T, VH extends Presenter<T>> SAFRecyclerAdapter<T, VH> create(List<T> list) {
        return new SAFRecyclerAdapter<T,VH>(list);
    }
    ...
    public SAFRecyclerAdapter<T, VH> createPresenter(Func2<ViewGroup, Integer, VH> presenter) {
    ...
    public SAFRecyclerAdapter<T, VH> bindViewHolder(Action3<VH, Integer, T> onBindViewHolder) {
